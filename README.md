...
- 🔘 **快捷磁贴**：下拉通知栏点按「开始录音」磁贴即可录制，锁屏可用（v1.8+，不需要 ADB 授权）
...
| 息屏几分钟后长按音量键没反应（亮屏后立即恢复） | 部分系统（实测小米 HyperOS / Android 16）会在息屏数分钟后**系统级停止向音量键监听派发事件**，属 ROM 行为，App 无法绕过。请改用**快捷磁贴**：下拉通知栏 → 编辑 → 添加「开始录音」，锁屏状态点一下即可，磁贴路径不经过音量键监听、不受此限制 |
